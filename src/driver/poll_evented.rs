...
///
/// Once the [`sys::Evented`] type is wrapped by `PollEvented`, it can be
...
///
/// `PollEvented` also allows receiving platform-specific `sys::event::Ready` events.
    ...
    E: Evented,
{
    ...
    pub fn poll_read_ready(
        ...
        if ret.is_empty() {
            ...
            Poll::Ready(Ok(sys::event::Ready::from_usize(cached)))
        }
    }
    ...
    pub fn poll_write_ready(
        &self,
        cx: &mut Context<'_>,
    ) -> Poll<Result<sys::event::Ready, io::Error>> {
        self.register()?;
