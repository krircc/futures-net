...
struct Reactor {
    /// Reuse the `sys::event::Events` value across calls to poll.
    events: sys::event::Events,

    /// State shared between the reactor and the handles.
    inner: Arc<Inner>,

    _wakeup_registration: sys::Registration,
}
...
const TOKEN_WAKEUP: sys::Token = sys::Token(MAX_SOURCES);
...
impl Reactor {
    ...
    fn new() -> io::Result<Reactor> {
        let io = sys::Poll::new()?;
        let wakeup_pair = sys::Registration::new2();

        io.register(
            &wakeup_pair.0,
            TOKEN_WAKEUP,
            sys::event::Ready::readable(),
            sys::event::PollOpt::level(),
        )?;

        Ok(Reactor {
            events: sys::event::Events::with_capacity(1024),
    ...
    fn poll(&mut self, max_wait: Option<Duration>) -> io::Result<()> {
        ...
        for event in self.events.iter() {
            ...
            if token == TOKEN_WAKEUP {
                self.inner
                    .wakeup
                    .set_readiness(sys::event::Ready::empty())
                    .unwrap();
            } else {
    ...
    fn dispatch(&self, token: sys::Token, ready: sys::event::Ready) {
        ...
        // lock's critical section.
        {
            ...
            if !(ready & (!sys::event::Ready::writable())).is_empty() {
                rd = io.reader.take();
            }
        }
...
impl HandlePriv {
    ...
    fn wakeup(&self) {
        if let Some(inner) = self.inner() {
            inner
                .wakeup
                .set_readiness(sys::event::Ready::readable())
                .unwrap();
        }
    }
...
impl Inner {
    ...
    fn add_source(&self, source: &dyn Evented) -> io::Result<usize> {
        ...
        self.io.register(
            source,
            sys::Token(aba_guard | key),
            sys::event::Ready::all(),
            sys::event::PollOpt::edge(),
        )?;
    ...
    fn register(&self, cx: &mut Context<'_>, token: usize, dir: Direction) {
        ...
        let (atomic_waker, ready) = match dir {
            Direction::Read => (&sched.reader, !sys::event::Ready::writable()),
            Direction::Write => (&sched.writer, sys::event::Ready::writable()),
        };
...
impl Direction {
    fn mask(&self) -> sys::event::Ready {
        match *self {
            Direction::Read => {
                // Everything except writable is signaled through read.
                sys::event::Ready::all() - sys::event::Ready::writable()
            }
            Direction::Write => sys::event::Ready::writable() | platform::hup(),
        }
    }
}

pub(crate) mod platform {
    use super::sys::event::Ready;
    use super::sys::UnixReady;
