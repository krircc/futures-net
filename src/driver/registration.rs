...
use super::sys::{self, event::Evented};
...
///
/// `Registration` also allows receiving platform-specific `sys::event::Ready` events.
...
impl Registration {
    ...
    ///
    /// [`poll_read_ready`]: #method.poll_read_ready
    pub fn take_read_ready(&self) -> io::Result<Option<sys::event::Ready>> {
    ...
    ///
    /// [`poll_write_ready`]: #method.poll_write_ready
    pub fn take_write_ready(&self) -> io::Result<Option<sys::event::Ready>> {
    ...
    fn poll_ready(
        ...
        direction: Direction,
    ) -> io::Result<Option<sys::event::Ready>> {
        let mut state = self.state.load(SeqCst);
